...
# --- DIAGNOSTIC SETTINGS (v2.0) ---
AI_SCAN_HEIGHT = 640        # Increased from 320 to 640 for better detection accuracy
ENABLE_MOTION_GATING = False # Disabled to ensure we don't skip anything
ENABLE_DEBUG_DUMP = True    # SAVES EVERYTHING > 0.10 SCORE TO PROVE AI IS WORKING
...
def video_worker(task, msg_queue):
    ...
    try:
        if AI_AVAILABLE:
            try:
                # Try Ryzen AI / DirectML first
                detector = NudeDetector(providers=['DmlExecutionProvider'])
                print("      (AI Loaded with DirectML Acceleration)")
            except:
                detector = NudeDetector()
                print("      (AI Loaded in Standard CPU Mode)")
        else:
            detector = None
    ...
    # Debug Folder
    debug_dir = os.path.join(OUTPUT_ROOT, "DEBUG_VIEW", video_name)
    ...
    hit_count = 0
    found_handles = set()
    prev_gray = None # For motion gating if enabled
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if frame_count % (skip_interval * 10) == 0:
                 print(f"      [{video_name}] Scanning Frame {frame_count}/{total_frames}...")
            ...
            if settings['scan_ocr']:
                try:
                    gray_ocr = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                    text = pytesseract.image_to_string(gray_ocr)
            ...
            if detector and not is_static:
                try:
                    ...
                    for det in detections:
                        ...
                        # DEBUG DUMP: Save ANYTHING > 10% confidence to verify AI is looking
                        if ENABLE_DEBUG_DUMP and score > 0.10:
                            d_sb = det['box']
                            d_box = [int(d_sb[0] * scale_x), int(d_sb[1] * scale_y), int(d_sb[2] * scale_x), int(d_sb[3] * scale_y)]
                            debug_frame = frame.copy()
                            cv2.rectangle(debug_frame, (d_box[0], d_box[1]), (d_box[0]+d_box[2], d_box[1]+d_box[3]), (0, 255, 255), 2)
                            cv2.putText(debug_frame, f"{label} {score:.2f}", (d_box[0], d_box[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                            cv2.imwrite(os.path.join(debug_dir, f"DEBUG_{frame_count}_{label}.jpg"), debug_frame)
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v2.0 - DIAGNOSTIC MODE")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v2.0", font=("Impact", 28)).pack(pady=(20, 10))
