...
class ArchivistPro(ctk.CTk):
    ...
    def _init_worker_grid(self, num_cores):
        ...
        # Calculate columns (approx 8 columns for 32 threads)
        cols = 8
        
        for i in range(num_cores):
            # Container for 1 worker
            f = ctk.CTkFrame(self.worker_grid_frame, width=130, height=110)
            f.grid(row=i//cols, column=i%cols, padx=5, pady=5)
            f.grid_propagate(False) # lock size
            
            # Preview Image Label
            img_lbl = ctk.CTkLabel(f, text=f"W{i}", fg_color="black", width=120, height=80)
            img_lbl.pack(pady=(5,0))
            
            # Status Text
            txt_lbl = ctk.CTkLabel(f, text="Idle", font=("Arial", 10), width=120)
            txt_lbl.pack()
