...
# --- HELPER: ROBUST PATH NORMALIZER (Windows Long Path Fix) ---
...
def video_worker(task, msg_queue, worker_idx):
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if motion_gate:
                # We reuse the 'gray' variable here
            ...
            # --- ROBUST OCR SCAN ---
            # Checks if OCR is enabled AND if we are on a 1-second interval
            if settings.get('scan_ocr', False) and (frame_count % int(fps) == 0):
                try:
                    # SAFETY CHECK: Define 'gray' if Motion Gating didn't create it
                    if 'gray' not in locals(): 
                        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                    
                    # 1. Threshold (Black & White for speed)
...
class ArchivistPro(ctk.CTk):
    ...
    def _init_worker_grid(self, num_cores):
        ...
        # 2. Dynamic Column Calculation (8 to 14 columns)
        ...
        for task_template in self.task_stack_local:
            ...
            try:
                ...
                for i in range(cores):
                    ...
                    chunk_task = {
                        "path": raw_path, "subject": subj, "settings": settings,
                        "start_frame": start, "end_frame": end, "is_chunk": True
                    }
