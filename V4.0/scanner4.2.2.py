...
import glob # Required for model auto-detection
...
# --- UPDATED FILTERS (Buttocks -> Warning) ---
FILTERS = {
    "CRITICAL": [
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v4.2.2 - FIXED UI")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v4.2.2", font=("Impact", 28)).pack(pady=(20, 10))
        ...
        # --- FIXED: ADDED FORCE STOP BUTTON BACK ---
    ...
    def _build_settings(self, parent):
        ...
        self.opt_preset = ctk.CTkOptionMenu(frame_perf, values=["Max Speed", "Balanced", "High Precision"], variable=self.var_preset, width=200)
    ...
    def add_videos(self):
        try:
            files = filedialog.askopenfilenames(parent=self, title="Select Video Files", filetypes=[("Video Files", "*.mp4 *.mkv *.avi *.mov *.wmv"), ("All Files", "*.*")])
            if files: self.stack_files(files)
        except Exception as e: messagebox.showerror("Error", f"Could not open file dialog:\n{e}")
    ...
    def force_refresh(self):
        self.check_messages(); self.update()
    ...
    def stack_files(self, files):
        ...
        for f in files:
            try:
                ...
                row = ctk.CTkFrame(self.queue_area); row.pack(fill="x", padx=5, pady=2)
                ctk.CTkLabel(row, text=f"{video_name}", anchor="w", width=300).pack(side="left", padx=10)
                est_lbl = ctk.CTkLabel(row, text="Wait...", width=100); est_lbl.pack(side="right", padx=10)
                bar = ctk.CTkProgressBar(row); bar.set(0); bar.pack(side="right", fill="x", expand=True, padx=10)
            ...
            except Exception as e: print(f"Error adding {f}: {e}")
        self.log(f"Added {len(files)} files to queue.")

    def log(self, msg):
        self.log_box.insert("end", f"{msg}\n"); self.log_box.delete("1.0", "end-1000l"); self.log_box.see("end")

    def check_messages(self):
        try:
            processed = 0
            while not self.msg_queue.empty() and processed < 100:
                m = self.msg_queue.get_nowait(); processed += 1
                if m['type'] == 'GRID_PREVIEW' and m['idx'] in self.grid_slots:
                    ...
                    if m['idx'] in self.grid_slots: self.grid_slots[m['idx']]['txt'].configure(text=m['file'][:15], text_color="green")
                ...
                elif m['type'] == 'LOG': self.log(m['msg'])
        except: pass 
    ...
    def generate_focus_tasks(self, cores):
        ...
        for task_template in self.task_stack_local:
            raw_path = task_template['path']; subj = task_template['subject']
            try:
                ...
                for i in range(cores):
                    ...
                    self.task_queue.put({"path": raw_path, "subject": subj, "settings": sett, "start_frame": start, "end_frame": end})
                while self.active_chunks > 0 and self.is_running: time.sleep(1)
                if not self.is_running: break
                time.sleep(2)
                self.log(f"Finished: {os.path.basename(raw_path)}")
            except Exception as e: self.log(f"Error chunking video: {e}")
