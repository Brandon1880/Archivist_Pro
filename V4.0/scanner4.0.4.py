...
import cv2
import os
os.environ['OMP_THREAD_LIMIT'] = '1'  # CRITICAL: Forces Tesseract to be single-threaded
...
def resolve_path(path):
    ...
    Priority 1: Try 8.3 Short Path (Most compatible with OpenCV/FFMPEG)
    ...
    # We use standard strings here to avoid SyntaxError with trailing backslashes
...
# --- HELPER: CLIP GENERATION (FIXED) ---
def generate_clip(video_path, start_time, duration, output_path):
    """
    Generates a 5-second clip around the start_time.
    Uses MoviePy v2.0+ syntax (.subclipped) and 'ultrafast' preset for speed.
    """
    try:
        with VideoFileClip(video_path) as clip:
            # Handle unknown duration safely
            ...
            # Cut the clip (Note: .subclipped is the new method in MoviePy v2.0)
            new_clip = clip.subclipped(start_t, end_time)
            
            # Write file (audio=False for speed, threads=4 to use CPU)
...
def video_worker(task, msg_queue, worker_idx):
    ...
    # Initial status update for the specific worker grid slot
    ...
    # LIVE FEED THROTTLE
    # Only update the grid once every 2 seconds to prevent UI lag with 32 workers
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if time.time() - last_preview_time > 2.0:
                try:
                    # Very small thumbnail for grid
                    preview = cv2.resize(frame, (120, 80))
                ...
                try:
                    ...
                    if hit_found:
                        ...
                        if hit_verified:
                            ...
                            # Define clip output path
                            clip_name = f_name.replace(".jpg", ".mp4")
                            clip_out = os.path.join(v_dir, clip_name)
                            
                            # Launch clip generation
                            # We use safe_path (original video) and timestamp
                            threading.Thread(
        ...
        if frames_processed_batch >= 50:
            ...
            frames_processed_batch = 0
        if frame_count % 50 == 0:
            import gc  # You can import it here if you didn't at the top
            gc.collect()
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v4.0 - COMMAND CENTER")
        ...
        # GRID VARS
        self.grid_slots = {} # {idx: {label_img, label_text}}
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v4.0", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def _init_worker_grid(self, num_cores):
        ...
        # For a Ryzen AI Max (32+ threads), we want wide layouts.
        # We set a floor of 8 and a ceiling of 14 columns based on thread count.
        cols = max(8, min(num_cores, 14))
        
        # 3. THE FIX: Grid Column Configuration
        # This tells the UI engine to distribute available horizontal space 
        # equally among all 'cols' columns.
        for i in range(cols):
            self.worker_grid_frame.grid_columnconfigure(i, weight=1)

        # 4. Generate Worker Cards
        for i in range(num_cores):
            # Container for 1 worker
            # REMOVED: width=130, height=110 (Let layout manager decide size)
            f = ctk.CTkFrame(self.worker_grid_frame)
            
            # sticky="ew" (East-West) forces the frame to stretch horizontally 
            # to fill its assigned grid cell.
            f.grid(row=i//cols, column=i%cols, padx=3, pady=3, sticky="ew")
            
            # Preview Image Label
            # Center it inside the now-flexible frame
    ...
    def stack_files(self, files):
        # 1. Capture Subject and Settings ONCE
        ...
        for f in files:
            # --- THE CRITICAL MISSING LOGIC ---
            task = {"path": f, "subject": subj, "settings": settings}
            
            # Add to the multiprocessing queue (for Standard Mode)
            self.task_queue.put(task)
            
            # Add to the local list (REQUIRED for Focus Mode)
            self.task_stack_local.append(task)
            print(f"DEBUG: Appended {os.path.basename(f)} to stack. Size is now: {len(self.task_stack_local)}") 
            # ----------------------------------
    ...
    def check_messages(self):
        while not self.msg_queue.empty():
            ...
            elif msg['type'] == 'WORKER_START':
                idx = msg['idx']
                fname = msg['file']
                if idx in self.grid_slots:
                    # Truncate long filenames
                    short_name = (fname[:15] + '..') if len(fname) > 15 else fname
                    self.grid_slots[idx]['txt'].configure(text=short_name, text_color="green")

            elif msg['type'] == 'WORKER_STOP':
                idx = msg['idx']
                if idx in self.grid_slots:
                    self.grid_slots[idx]['txt'].configure(text="Idle", text_color="gray")
                    self.grid_slots[idx]['img'].configure(image=None, text=f"W{idx}")
            ...
            # --- NORMAL UI UPDATES ---
                ...
                if v_name in self.prog_data:
                    self.prog_data[v_name]['total'] = t_frames
                    self.prog_data[v_name]['start_time'] = time.time() 
                ...
                if v_name in self.prog_data and v_name in self.prog_bars:
                    ...
                    if data['start_time'] > 0:
                        ...
                        if pct > 0.01:
                            ...
                            if rem_seconds > 3600:
                                etc_str = f"{int(rem_seconds//3600)}h {int((rem_seconds%3600)//60)}m"
                            else:
