...
# OPTIMIZATION: Set this to a RAM Disk (e.g., "R:\\Archived_Scans") for maximum speed
# If no RAM disk, keep it as "Archived_Scans"
...
def video_worker(task, msg_queue):
    # DEBUG: Print to console so user sees it is running
    print(f"   -> Worker assigned to: {os.path.basename(task['path'])}")
    ...
    # Re-initialize AI inside the worker process
    # OPTIMIZATION: Try to force DirectML for Ryzen AI / GPU acceleration
    try:
        if AI_AVAILABLE:
            try:
                # Attempt to load with DirectML provider (requires onnxruntime-directml)
                detector = NudeDetector(providers=['DmlExecutionProvider'])
            except:
                # Fallback to standard CPU execution if DirectML fails
                detector = NudeDetector()
        else:
            detector = None
    except Exception as e:
        print(f"!!! AI CRASHED IN WORKER: {e}")
        return
                ...
                try:
                    ...
                    for h in handles: 
                        if h not in found_handles:
                            found_handles.add(h)
                            if settings['debug']:
                                msg_queue.put({'type': 'LOG', 'msg': f"[OCR] {h}"})
                except Exception: pass # OCR errors are non-critical
                ...
                if hit_verified:
                    ...
                    try:
                        with VideoFileClip(path) as video:
                            ...
                            clip.write_videofile(os.path.join(ver_dir, clip_name), codec="libx264", audio=False, logger=None)
                    except: pass
...
# --- TOP LEVEL WRAPPER (REQUIRED FOR WINDOWS MULTIPROCESSING) ---
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v1.7 - Ryzen AI Max Edition")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v1.7", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def start_processing(self):
        ...
        # OPTIMIZATION: Use all logical processors for your Ryzen 395+
    ...
    def run_dynamic_pool(self, cores):
        # NOTE: worker_wrapper is now Global (Top-Level) so it can be pickled
        self.pool = []
