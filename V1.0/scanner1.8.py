...
import time
import queue 
import numpy as np  # Required for Motion Gating
...
# Set this to a path on your RAM Disk (e.g. "R:\\Archived_Scans") for max speed
OUTPUT_ROOT = "Archived_Scans" 

# --- PERFORMANCE TUNING ---
BATCH_SIZE = 4            # How many frames to send to AI at once (Increase if you have 128GB RAM)
AI_SCAN_HEIGHT = 320      # Downscale AI input to this height (Massive speedup)
MOTION_THRESHOLD = 25000  # Sensitivity for ignoring static frames (Higher = ignores more)
...
def verify_crop(frame, box, detector, thresh):
    ...
    # Verification is always single-pass for accuracy
...
def video_worker(task, msg_queue):
    ...
    # Batch Buffers
    batch_frames = []      # The small frames for AI
    batch_indices = []     # Frame numbers
    batch_timestamps = []  # Timestamps
    batch_originals = []   # Original frames (for saving)
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            # --- FEATURE 2: MOTION GATING ---
            # Check if frame changed enough to warrant a scan
            ...
            if prev_gray is not None:
                ...
                if change_score < MOTION_THRESHOLD:
                    is_static = True # Skip this frame, it's basically the same image
            
            prev_gray = gray
            
            # Report Progress & OCR (Still done even if static)
            ...
            if detector and not is_static:
                ...
                batch_originals.append(frame) # Keep ref to original for saving

                # --- PROCESS BATCH IF FULL ---
                if len(batch_frames) >= BATCH_SIZE:
                    
                    # FEATURE 3: BATCH INFERENCE
                    # We send a list of images to detector.detect()
                    try:
                        # NudeNet detect() typically handles list of images
                        batch_results = detector.detect(batch_frames)
                        ...
                        for i, detections in enumerate(batch_results):
                            ...
                            # Scaling Factors for this frame
                            ...
                            for det in detections:
                                ...
                                if settings['double_check']:
                                    if verify_crop(batch_originals[i], real_box, detector, settings['threshold2']):
                                        hit_verified = True
                                else:
                                    hit_verified = True
                                ...
                                x,y,w,h = real_box
                                cv2.rectangle(batch_originals[i], (x, y), (x+w, y+h), (0, 0, 255), 2)
                                break # Stop after first valid hit per frame

                            # Save if hit
                            if hit_found:
                                hit_count += 1
                                current_orig = batch_originals[i]
                                current_ts = batch_timestamps[i]
                                current_idx = batch_indices[i]
                                
                                cat_folder = label_map.get(label_found.upper(), "UNCATEGORIZED")
                                s = int(current_ts)
                                t_str = f"{s//3600:02d}-{(s%3600)//60:02d}-{s%60:02d}"
                                f_name = f"{label_found.replace(' ', '_').title()}_{t_str}_F{current_idx}.jpg"
                                
                                # First Pass Save
                                fp_dir = os.path.join(base_root, "FIRST_PASS", cat_folder)
                                os.makedirs(fp_dir, exist_ok=True)
                                cv2.imwrite(os.path.join(fp_dir, f_name), current_orig)
                                
                                # Verified Save
                                if hit_verified:
                                    v_dir = os.path.join(base_root, "VERIFIED", cat_folder)
                                    os.makedirs(v_dir, exist_ok=True)
                                    cv2.imwrite(os.path.join(v_dir, f_name), current_orig)
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v1.8 - SPEED DEMON")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v1.8", font=("Impact", 28)).pack(pady=(20, 10))
