...
def verify_crop(frame, box, detector, thresh):
    ...
    for d in detections:
        # FIX: Check 'class' first, then 'label'
        score = d.get('score', 0)
        ...
        if frame_count % skip_interval == 0:
            ...
            if detector:
                ...
                for det in detections:
                    # FIX: Robust Key Check (checks 'class', then 'label')
                    label = det.get('class', det.get('label', 'UNKNOWN'))
                    score = det.get('score', 0)
                    
                    # FIX: Force Uppercase for Filter Matching
...
class ArchivistPro(ctk.CTk):
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v1.0", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def _build_dashboard(self, parent):
        ...
        # Queue
        ctk.CTkLabel(parent, text="Batch Queue").pack(anchor="w", padx=10)
        ...
        self.prog_bar.set(0)
        self.prog_bar.pack(fill="x", padx=10, pady=10)
    ...
    def get_active_filters(self):
        ...
        for lbl, var in self.chk_filters.items():
            # IMPORTANT: We are appending the KEY (e.g. FEMALE_BREAST_EXPOSED)
            # This must match what the AI returns (which we now force to uppercase)
    ...
    def stack_files(self, files):
        ...
        for f in files:
            ...
            txt = f"[{subj}] {f_name} | Thr: {settings['threshold1']:.2f}"
            ctk.CTkLabel(self.queue_area, text=txt, anchor="w", font=("Consolas", 12)).pack(fill="x", padx=5, pady=2)
    ...
    def start_processing(self):
        ...
        self.tabs.set("  LIVE DASHBOARD  ") # Auto switch to dashboard
        
        # NOTE: If you experience crashes, set this to 1
    ...
    def generate_pdf(self, results):
        ...
        pdf.cell(0, 10, "Archivist Pro v1.0 Report", ln=1, align='C')
        pdf.ln(10)
        pdf.set_font("Arial", size=10)
        for r in results:
            pdf.set_fill_color(240, 240, 240)
            pdf.cell(0, 10, f"Subject: {r['subject']} | File: {r['file']}", ln=1, fill=True)
            pdf.cell(0, 10, f"Hits: {r['hits']} | Handles: {', '.join(r['handles'])}", ln=1)
            pdf.ln(5)
