...
def video_worker(task, msg_queue, worker_idx):
    ...
    while True:
        ...
        if frames_since_update >= ui_update_interval:
            # We send 'frames_since_update' to let UI know how many frames we just crunched
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v4.6 - FRAME COUNTERS")
    ...
    def stack_files(self, files):
        ...
        for f in files:
            ...
            self.prog_data[os.path.basename(f)] = {'total': 1, 'done': 0} # Default 1 to avoid div/0
    ...
    def init_file_ui(self, path, label_widget):
        # Background: Open video, get true frame count, update label
        try:
            cap = cv2.VideoCapture(resolve_path(path))
            if cap.isOpened():
                total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                self.prog_data[os.path.basename(path)]['total'] = total
                label_widget.configure(text=f"0 / {total:,}") # Display: "0 / 15,400"
            cap.release()
        except: pass
