...
def video_worker(task, msg_queue):
    ...
    # Don't send "STATUS" message here, it overwrites the progress bar.
    # We let the TICK updates handle the UI text.
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if time.time() - last_preview_time > 1.0:
                try:
                    # Resize specifically for UI (keep it small to be fast)
                    preview = cv2.resize(frame, (300, 200))
                    # Encode to jpg bytes for safe transmission
                    _, buf = cv2.imencode(".jpg", preview)
                    msg_queue.put({'type': 'PREVIEW', 'data': buf.tobytes()})
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.9 - LIVE FEED FIX")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.9", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def _build_dashboard(self, parent):
        ...
        self.lbl_feed.pack(side="left", expand=True, fill="both", padx=5, pady=5)
        
        # HIT LABEL
        self.lbl_hit = ctk.CTkLabel(preview_frame, text="[ Latest Critical Hit ]", text_color="gray")
        self.lbl_hit.pack(side="right", expand=True, fill="both", padx=5, pady=5)
