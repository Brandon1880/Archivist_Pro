...
import queue 
import numpy as np 
import ctypes # REQUIRED FOR SHORT PATH FIX
...
# --- HELPER: WINDOWS SHORT PATH (THE EMOJI FIX) ---
def get_short_path(long_path):
    # Converts "Video 🥺.mp4" -> "VIDEO~1.MP4"
    # This allows OpenCV C++ backend to read files with emojis/unicode
    try:
        ...
        return long_path # Fallback if fails
...
def video_worker(task, msg_queue):
    ...
    safe_path = get_short_path(raw_path) # <--- THIS FIXES THE EMOJIS
    ...
    # Use basename of raw_path for logging so it looks nice in UI
    ...
    # print(f"      (Internal Path: {safe_path})") # Uncomment to see the DOS name
    ...
    video_name = os.path.splitext(display_name)[0]
    # Sanitize folder name (remove emojis from folder creation to be safe)
    clean_video_name = re.sub(r'[^\w\-_\. ]', '_', video_name)
    base_root = os.path.join(OUTPUT_ROOT, subject, clean_video_name)
    os.makedirs(base_root, exist_ok=True)

    # --- VIDEO LOADING ---
    try:
        # FORCE FFMPEG (Proven to work by diagnostic)
        cap = cv2.VideoCapture(safe_path, cv2.CAP_FFMPEG)
        
        if not cap.isOpened():
            # Try one fallback just in case
            cap = cv2.VideoCapture(safe_path)
            
        if not cap.isOpened():
            msg_queue.put({'type': 'LOG', 'msg': f"❌ ERROR: Could not open {display_name}"})
            print(f"!!! FAILED TO OPEN: {safe_path}")
            return
    ...
    while True:
        ...
        if frame is None or frame.size == 0:
            ...
            if no_frame_counter > 50:
                print(f"!!! ABORT: Too many bad frames in {display_name}")
                break
            frame_count += 1
            continue
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.4 - SHORT PATH FIX")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.4", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def test_file_compatibility(self):
        f = filedialog.askopenfilename(title="Select Problematic Video", filetypes=[("Video", "*.mp4 *.mkv *.avi *.mov")])
        if not f: return
        
        # Get safe path for test
        try:
            buffer = ctypes.create_unicode_buffer(1024)
            ctypes.windll.kernel32.GetShortPathNameW(f, buffer, 1024)
            safe_path = buffer.value
        except:
            safe_path = f

        report = f"Diagnostic Report for:\n{os.path.basename(f)}\n\n"
        success = False
        
        backends = [
            (cv2.CAP_FFMPEG, "FFMPEG"),
            (cv2.CAP_MSMF, "MS Media Foundation"),
            (cv2.CAP_DSHOW, "DirectShow")
        ]
        
        for backend, name in backends:
            try:
                cap = cv2.VideoCapture(safe_path, backend)
                if cap.isOpened():
                    ret, frame = cap.read()
                    if ret and frame is not None:
                        ...
                        report += f"❌ {name}: Opened but failed to read frame.\n"
                else:
                    report += f"❌ {name}: Failed to open container.\n"
                cap.release()
            except Exception as e:
                report += f"❌ {name}: Crash - {e}\n"

        if success:
            messagebox.showinfo("Diagnostic Success", report)
        else:
            messagebox.showerror("Diagnostic Failure", report + "\nRECOMMENDATION: Convert this video to standard MP4 (H.264) using Handbrake or ffmpeg.")
    ...
    def generate_focus_tasks(self, cores):
        ...
        for task_template in self.task_stack_local:
            ...
            # Use safe path for metadata reading too
            try:
                buffer = ctypes.create_unicode_buffer(1024)
            ...
            try:
                ...
                for i in range(cores):
                    ...
                    chunk_task = {
                        "path": raw_path, # Worker converts this to safe path again
                        "subject": subj,
                        "settings": settings,
                        "start_frame": start,
                        "end_frame": end,
