...
# FPS Baseline for Ryzen AI Max (Adjust if needed)
...
def video_worker(task, msg_queue):
    ...
    # --- RESOLUTION LOGIC (Restored from v3.6) ---
    ...
    if start_frame == 0:
        msg_queue.put({'type': 'STATUS', 'video': display_name, 'status': f'Scan: {scan_preset}'})
    ...
    # We rely on UI aggregator for init, but send update anyway
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if detector and not is_static:
                try:
                    # 1. DOWNSCALE (Using the restored scan_height variable)
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.7 - CORRECTED MERGE")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.7", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def _build_settings(self, parent):
        ...
        # Hidden Manual Resolution (overridden by preset unless we modify get_settings)
        # We'll just rely on presets for simplicity now.
    ...
    def stack_files(self, files):
        ...
        for f in files:
            ...
            # --- IMMEDIATE METADATA READ (New Feature) ---
            try:
                buffer = ctypes.create_unicode_buffer(1024)
                ctypes.windll.kernel32.GetShortPathNameW(os.path.abspath(f), buffer, 1024)
                safe_path = buffer.value
