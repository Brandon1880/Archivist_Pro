...
def video_worker(task, msg_queue):
    ...
    # Send Initialization Message only from the first chunk or main worker
    # This sets the "Total Frames" for the UI aggregator
    if start_frame == 0:
        msg_queue.put({
            'type': 'INIT_VIDEO', 
            'video': video_name, 
    ...
    frames_processed_batch = 0 # Local counter for batch reporting
    prev_gray = None
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if settings['scan_ocr'] and not is_static:
                try:
                    ...
                    # Just debug print handles in chunks to avoid queue locking
        ...
        # REPORTING LOGIC (New v3.1 Aggregation)
        # Instead of sending absolute %, we send "I processed X frames"
        # We batch these updates (every 50 frames) to keep the UI responsive
        frames_processed_batch += 1
        if frames_processed_batch >= 50:
            msg_queue.put({
                'type': 'TICK', 
                'video': video_name, 
                'count': frames_processed_batch
            })
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.1 - ETC & AGGREGATOR")
        ...
        self.prog_bars = {}         # Widget references
        self.prog_data = {}         # Tracking data: {video_name: {'total': X, 'done': Y, 'start': T}}
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.1", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def check_messages(self):
        while not self.msg_queue.empty():
            ...
            # 1. INITIALIZE TRACKER FOR VIDEO
            ...
            # 2. TICK UPDATES (Aggregated Progress)
            elif msg['type'] == 'TICK':
                v_name = msg['video']
                count = msg['count']
    ...
    def start_processing(self):
        ...
        if self.var_focus.get():
            ...
            # Clear standard queue to prevent duplicates
    ...
    def generate_focus_tasks(self, cores):
        ...
        for task_template in self.task_stack_local:
            ...
            try:
                ...
                # FALLBACK: If total frames is 0 (error reading metadata), disable chunking for this file
                if total_frames <= 0:
