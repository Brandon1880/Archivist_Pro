...
def verify_crop(frame, box, detector, thresh):
    ...
    # This crop comes from the ORIGINAL High-Res frame
...
def video_worker(task, msg_queue):
    ...
    if start_frame == 0:
        msg_queue.put({'type': 'STATUS', 'video': display_name, 'status': f'Scanning @ {scan_height}p'})
    ...
    while True:
        ...
        if frame_count % skip_interval == 0:
            ...
            if detector and not is_static:
                try:
                    # 1. DOWNSCALE FOR SPEED (The "Scan Low" step)
                    ...
                    for det in detections:
                        ...
                        # Map small box to ORIGINAL coordinates
                        sb = det['box']
                        ...
                        # 2. VERIFY ON ORIGINAL (The "Check High" step)
                        # We pass the full resolution 'frame' here, ensuring max accuracy
                    ...
                    if hit_found:
                        ...
                        # 3. SAVE ORIGINAL (The "Save High" step)
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.6 - PRESETS & RESOLUTION")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.6", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def _build_settings(self, parent):
        ...
        ctk.CTkLabel(frame_perf, text="PERFORMANCE PRESETS", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        ctk.CTkLabel(frame_perf, text="Max Speed: 240p Scan (Blazing Fast)\nBalanced: 480p Scan (Recommended)\nHigh Precision: 720p Scan (Slow)", 
                     font=("Arial", 10), text_color="gray").pack(pady=5)

        ctk.CTkLabel(frame_perf, text="ADVANCED TUNING", font=("Arial", 12, "bold")).pack(pady=(20,5))
        
        self.var_focus = ctk.BooleanVar(value=False)
        ctk.CTkSwitch(frame_perf, text="FOCUS MODE (All Cores/1 Video)", variable=self.var_focus, progress_color="#e74c3c").pack(anchor="w", padx=20, pady=5)
        
        self.var_motion = ctk.BooleanVar(value=False)
        ctk.CTkSwitch(frame_perf, text="Motion Gating (Skip Static)", variable=self.var_motion).pack(anchor="w", padx=20, pady=5)
