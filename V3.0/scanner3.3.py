...
def video_worker(task, msg_queue):
    ...
    for backend, name in backends:
        try:
            temp_cap = cv2.VideoCapture(path, backend)
            if temp_cap.isOpened():
                ...
                if ret and test_frame is not None:
                    ...
                    if start_frame == 0:
                        print(f"      [SUCCESS] Opened {video_name} using {name}")
                    break
                else:
                    temp_cap.release()
        except:
    ...
    if cap is None:
        msg_queue.put({'type': 'LOG', 'msg': f"❌ CRITICAL ERROR: Could not open {video_name} with ANY codec."})
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v3.3 - UNIVERSAL READER")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v3.3", font=("Impact", 28)).pack(pady=(20, 10))
    ...
    def test_file_compatibility(self):
        ...
        if not os.path.exists(f):
            messagebox.showerror("Test Failed", f"File path not found by OS:\n{f}")
            return
        ...
        for backend, name in backends:
            try:
                cap = cv2.VideoCapture(f, backend)
                if cap.isOpened():
                    ret, frame = cap.read()
