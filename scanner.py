...
# ==========================================
#        WORKER ENGINE (CORRUPTION PROOF)
# ==========================================
def video_worker(task, msg_queue, worker_idx):
    ...
    try:
        ...
        # --- NEW: CORRUPTION TOLERANCE ---
        bad_frame_streak = 0 
        MAX_BAD_STREAK = 50 # Allow 50 bad frames before quitting
        ...
        while True:
            ...
            # --- FIX: ROBUST SKIP LOGIC ---
            if not ret or frame is None:
                bad_frame_streak += 1
                if bad_frame_streak >= MAX_BAD_STREAK:
                    # We hit 50 bad frames in a row, assume end of file
                    break 
                frame_count += 1
                continue # Skip this loop iteration and try next frame
            
            # If we got a good frame, reset the streak counter
            bad_frame_streak = 0
        ...
        if frames_since_update > 0: msg_queue.put({'type': 'TICK', 'video': display_name, 'count': frames_since_update})
        if not end_frame: msg_queue.put({'type': 'DB_UPDATE', 'file': display_name})
        cap.release()

    except Exception as e:
        msg_queue.put({'type': 'LOG', 'msg': f"❌ Worker {worker_idx} CRASHED: {e}"})
...
class ArchivistPro(ctk.CTk):
    def __init__(self):
        ...
        self.title("Archivist Pro v4.7 - CORRUPTION PROOF")
    ...
    def _setup_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="ARCHIVIST PRO v4.7", font=("Impact", 28)).pack(pady=20)
    ...
    def stack_files(self, files):
        ...
        for f in files:
            ...
            est_lbl = ctk.CTkLabel(row, text="Loading...", width=150); est_lbl.pack(side="right")
            bar = ctk.CTkProgressBar(row); bar.set(0); bar.pack(side="right", fill="x", expand=True)
            self.prog_bars[os.path.basename(f)] = {'bar': bar, 'lbl': est_lbl}
            self.prog_data[os.path.basename(f)] = {'total': 1, 'done': 0} 
            threading.Thread(target=self.init_file_ui, args=(f, est_lbl), daemon=True).start()
            cnt += 1
        self.log_box.insert("end", f"Queued {cnt} files. Click START to begin.\n")

    def init_file_ui(self, path, label_widget):
        try:
            cap = cv2.VideoCapture(resolve_path(path))
            if cap.isOpened():
                total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                self.prog_data[os.path.basename(path)]['total'] = total
                label_widget.configure(text=f"0 / {total:,}") 
            cap.release()
        except: pass
        ...
        try:
            ...
            while not self.msg_queue.empty() and cnt < 100:
                ...
                if m['type'] == 'GRID_PREVIEW' and m['idx'] in self.grid_slots:
                    ...
                    self.prog_data[m['video']]['done'] += m['count']
                    done = self.prog_data[m['video']]['done']
                    total = self.prog_data[m['video']]['total']
                    if total > 0: self.prog_bars[m['video']]['bar'].set(done / total)
                    self.prog_bars[m['video']]['lbl'].configure(text=f"{done:,} / {total:,}")
                elif m['type'] == 'HIT':
                    try: self.lbl_hit.configure(image=ctk.CTkImage(Image.open(m['path']), size=(300, 200)), text="")
                    except: pass
                elif m['type'] == 'LOG': 
                    if self.var_debug.get() or "Error" in m['msg'] or ">>>" in m['msg'] or "✔" in m['msg']:
    ...
    def generate_focus_tasks(self, cores):
        ...
        for t in self.task_stack_local:
            try:
                self.msg_queue.put({'type': 'LOG', 'msg': f">>> STARTING: {os.path.basename(t['path'])}"})
                ...
                if self.is_running: self.msg_queue.put({'type': 'LOG', 'msg': f"✔ COMPLETED: {os.path.basename(t['path'])}"})
            except: pass
